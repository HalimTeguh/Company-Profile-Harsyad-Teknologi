@if (session($key ?? 'error'))
    <div class="alert alert-danger alert-dismissible fade show" role="alert" id="alert-danger" style="display: flex; justify-content: space-between; align-items: center;">
        <span>{{ session($key ?? 'error') }}</span>
        <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" fill="none" viewBox="0 0 24 24" class="icon-lg" style="cursor: pointer;" onclick="document.getElementById('alert-danger').style.display='none';">
